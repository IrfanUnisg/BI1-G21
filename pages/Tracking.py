...
data = {
    ...
    'Gesparte Kosten (CHF)': [45, 48, 60, 67, 75, 83, 90, 93, 75, 67, 57, 52]
}
...
st.bar_chart(df.set_index('Monat')['Gesparte Kosten (CHF)'])
...
total_savings = df['Gesparte Kosten (CHF)'].sum()

st.metric("Gesamte erzeugte Energie (kWh)", f"{total_energy} kWh")
st.metric("Gesamt eingesparte Kosten (CHF)", f"{total_savings} CHF")
